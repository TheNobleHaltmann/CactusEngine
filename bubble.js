...
class HexGrid {
  ...
  clearAllRows() {
    for( var i = 0; i < this.grid.length; i++ ) {
      for( var j = 0; j < this.grid[i].length; j++ ) {
        remove( this.grid[i][j], engine.objects );
      }
    }
    remove( activeBubble, engine.objects );
    remove( nextBubble, engine.objects );
  }
}
...
function newGame() {
  ...
  remove( startGameListener, engine.objects );
...
function gameOver() {
  ...
  remove( guideLine, engine.objects );
