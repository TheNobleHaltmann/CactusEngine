...
function startup() {
  ...
  pizzaSpr = new drawnObject( "pizza", gridCellSize, gridCellSize, 0, 0, "./pizza.png");
  badPizzaSpr = new drawnObject( "bad pizza", gridCellSize, gridCellSize, 0, 0, "./crapPizza.png");
  cactusSpr = new drawnObject( "snake", gridCellSize, gridCellSize, 0, 0, "./cactusSmall.png" );
  doubleFireSpr = new drawnObject( "snake", gridCellSize, gridCellSize, 0, 0, "./doubleFireSmall.png" );
  ...
  highScoreText = new Text( "High Score: " + highScore, 2 * ( engine.canv.width / 3 ), engine.canv.height - 9 );
  //add "click to start game" to engine
  clickToStart = new Text( "Click to start game.", engine.canv.width / 3 + gridCellSize/2, engine.canv.height / 2, "50px Papyrus" );
  ...
  pizzaTimer = new drawnObject( "pizzaTimer", 0, 0, -50, -50, pizzaSpr.img.src );
...
function nonRandomlyPlace( o, x, y ) {
  grid[x][y].occupied = true;
  grid[x][y].occupiedBy = o;
  o.x = x * (gridCellOutlineSize+gridCellSize) + o.img.width/2 - 1;
  o.y = y * (gridCellOutlineSize+gridCellSize) + o.img.height/2 - 1;
...
//this function should be networked
function placePizza( x, y ) {
  grid[x][y].occupied = true;
  grid[x][y].occupiedBy = pizzaSpr;
  pizzas.push( grid[x][y] );
  engine.addObject( grid[x][y] );
  grid[x][y].occupiedBy.x = x * (gridCellOutlineSize+gridCellSize) + grid[x][y].occupiedBy.img.width/2 - 1;
  grid[x][y].occupiedBy.y = y * (gridCellOutlineSize+gridCellSize) + grid[x][y].occupiedBy.img.height/2 - 1;
}
...
function newPizza() {
  ...
  placePizza( i, j );
  engine.MPconn.send({"funct":6,"event":{"x":i, "y":j }});
...
function newGame() {
  ...
  for( var i = 0; i < numSnakes; i++ ) {
    ...
    if( i == 0 ) {
      nonRandomlyPlace( snakeHeads[i], 1, rows/2 );
    } else {
      nonRandomlyPlace( snakeHeads[i], columns-2, rows/2 );
    }
...
var playerNumber;
function hostButtonFunc() {
  playerNumber = 0;

  engine.hostMP(startup);
}

function joinButtonFunc() {
  playerNumber = 1;
  
  engine.connectMP(startup);
}
