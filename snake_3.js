...
function decrementSnakeCount( c ) {
  ...
  if( c.snakeCount <= 0 ) {
	if( c.occupiedBy != null && c.occupiedBy.name == "pizza" ) {
		...
		engine.objects.push( c.occupiedBy );
		newPizza();
	} else {
		clearCell( c );
	}
...
function newPizza() {
 ...
 if( grid[x][y].occupied == false ) {
  ...
  engine.objects.push( grid[x][y].occupiedBy );
 } else {
...
function putSnakeInCell( cell ) {
  ...
  engine.objects.push( cell.occupiedBy );
  cell.snakeCount = snakeLength+1;
  bodyParts.push( cell );
}

var gameController = new Line( -1, -1, -1, -1 );
gameController.update = update;
engine.addObject( gameController );
